...
import ai.PassiveAI;
import ai.core.AI;
import ai.jni.PLAPResponse;
import ai.jni.PLAPResponses;
 ...
 */
public class JNIGridnetPLAPVecClient {
	
	/** Clients of 1 Python/JNI agent vs. 1 Java bot */
    public JNIGridnetPLAPClient[] clients;
    /** Clients of 1 Python/JNI agent vs. 1 Python/JNI agent */
    public JNIGridnetPLAPClientSelfPlay[] selfPlayClients;
    /** Clients of 1 Java bot vs. 1 Java bot */
    public JNIBotPLAPClient[] botClients;
     ...
     * @throws Exception
     */
    public JNIGridnetPLAPVecClient(int a_num_selfplayenvs, int a_num_envs, int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths, 
        ...
        selfPlayClients = new JNIGridnetPLAPClientSelfPlay[a_num_selfplayenvs/2];
        for (int i = 0; i < selfPlayClients.length; i++) {
            selfPlayClients[i] = new JNIGridnetPLAPClientSelfPlay(a_rfs, a_micrortsPath, mapPaths[i*2], a_utt, partialObs);
        }
        clients = new JNIGridnetPLAPClient[a_num_envs];
        for (int i = 0; i < clients.length; i++) {
            clients[i] = new JNIGridnetPLAPClient(a_rfs, a_micrortsPath, mapPaths[a_num_selfplayenvs+i], a_ai2s[i], a_utt, partialObs);
        }

        // initialize storage
        PLAPResponse r = new JNIGridnetPLAPClient(a_rfs, a_micrortsPath, mapPaths[0], new PassiveAI(a_utt), a_utt, partialObs).reset(0);
        ...
        responses = new PLAPResponses(null, null, null, null);
        rs = new PLAPResponse[s1];
    }
     ...
     * @throws Exception
     */
    public JNIGridnetPLAPVecClient(int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths,
        ...
        // initialize clients
        botClients = new JNIBotPLAPClient[a_ai2s.length];
        for (int i = 0; i < botClients.length; i++) {
            botClients[i] = new JNIBotPLAPClient(a_rfs, a_micrortsPath, mapPaths[i], a_ai1s[i], a_ai2s[i], a_utt, partialObs);
        }
        PLAPResponse r = new JNIGridnetPLAPClient(a_rfs, a_micrortsPath, mapPaths[0], new PassiveAI(a_utt), a_utt, partialObs).reset(0);
        int s2 = r.observation.length; 
        int s3 = r.observation[0].length;
        int s4 = r.observation[0][0].length;
        responses = new PLAPResponses(null, null, null, null);
        rs = new PLAPResponse[a_ai2s.length];
    ...
    public PLAPResponses reset(int[] players) throws Exception {
        if (botClients != null) {
            for (int i = 0; i < botClients.length; i++) {
    ...
    public PLAPResponses gameStep(int[][][] action, int[] players) throws Exception {
        if (botClients != null) {
            for (int i = 0; i < botClients.length; i++) {
    ...
    public void close() throws Exception {
        if (clients != null) {
            for (JNIGridnetPLAPClient client : clients) {
                client.close();
            }
        }
        if (selfPlayClients != null) {
            for (JNIGridnetPLAPClientSelfPlay client : selfPlayClients) {
                client.close();
            }
        }
        if (botClients != null) {
