...
import ai.PassiveAI;
import ai.core.AI;
import ai.jni.PLARResponse;
import ai.jni.PLARResponses;
 ...
 */
public class JNIGridnetPLARVecClient {
	
	/** Clients of 1 Python/JNI agent vs. 1 Java bot */
    public JNIGridnetPLARClient[] clients;
    /** Clients of 1 Python/JNI agent vs. 1 Python/JNI agent */
    public JNIGridnetPLARClientSelfPlay[] selfPlayClients;
    /** Clients of 1 Java bot vs. 1 Java bot */
    public JNIBotPLARClient[] botClients;
     ...
     * @throws Exception
     */
    public JNIGridnetPLARVecClient(int a_num_selfplayenvs, int a_num_envs, int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths, 
        ...
        selfPlayClients = new JNIGridnetPLARClientSelfPlay[a_num_selfplayenvs/2];
        for (int i = 0; i < selfPlayClients.length; i++) {
            selfPlayClients[i] = new JNIGridnetPLARClientSelfPlay(a_rfs, a_micrortsPath, mapPaths[i*2], a_utt, partialObs);
        }
        clients = new JNIGridnetPLARClient[a_num_envs];
        for (int i = 0; i < clients.length; i++) {
            clients[i] = new JNIGridnetPLARClient(a_rfs, a_micrortsPath, mapPaths[a_num_selfplayenvs+i], a_ai2s[i], a_utt, partialObs);
        }
     ...
     * @throws Exception
     */
    public JNIGridnetPLARVecClient(int a_max_steps, RewardFunctionInterface[] a_rfs, String a_micrortsPath, String[] a_mapPaths,
        ...
        // initialize clients
        botClients = new JNIBotPLARClient[a_ai2s.length];
    ...
    public PLARResponses reset(int[] players) throws Exception {
        if (botClients != null) {
            for (int i = 0; i < botClients.length; i++) {
    ...
    public PLARResponses gameStep(int[][][] action, int[] players) throws Exception {
        if (botClients != null) {
            for (int i = 0; i < botClients.length; i++) {
    ...
    public void close() throws Exception {
        if (clients != null) {
            for (JNIGridnetPLARClient client : clients) {
                client.close();
            }
        }
        if (selfPlayClients != null) {
            for (JNIGridnetPLARClientSelfPlay client : selfPlayClients) {
                client.close();
            }
        }
        if (botClients != null) {
