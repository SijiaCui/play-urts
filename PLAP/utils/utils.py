...
def load_args():
    ...
    with open("/root/desc/play-urts/PLAP/configs/configs.yaml", "r") as f:
...
class path_planning:
    DIRECTION_INDEX_MAPPING = {"north": 0, "east": 1, "south": 2, "west": 3}
    ...
    def _get_neighbors_with_directions(
    ...
    def get_shortest_path(
        self, location: Tuple[int, int], tg_location: Tuple[int, int]
    ...
    def get_path_nearest(
        self, location: Tuple[int, int], targets: List[Tuple[int, int]]
        ...
        while queue:
            ...
            for _, neighbor in self._get_neighbors_with_directions(pos):
                if neighbor not in visited:
                    if (
                        self.valid_map[neighbor[0]][neighbor[1]] == 1
                        or neighbor in target_set
                    ):
                        visited.add(neighbor)
...
def parse_task(text: str) -> list:
    import ast
    import re
    from PLAP.grounding import TASK_SPACE
...
def update_tasks(tasks: List[Tuple], situation, obs_dict):
    import PLAP.utils as utils
    from PLAP.grounding import TASK_BUILD_BUILDING, TASK_PRODUCE_UNIT, TASK_ATTACK_ENEMY
...
def check_task(tasks, obs_dict, situation, aggressive=False):
    import PLAP.utils as utils
    from PLAP.grounding import TASK_HARVEST_MINERAL, TASK_ATTACK_ENEMY
    ...
    for task in tasks[:]:
        if task[0] == TASK_HARVEST_MINERAL:
            is_exist_mine = (
                ...
                and obs_dict["units"][task[1]]["type"] == "resource"
            )
