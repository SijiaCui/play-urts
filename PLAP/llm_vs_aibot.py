...
import yaml
import numpy as np
from PLAP.grounding import obs_2_text, script_mapping
from PLAP.utils.utils import (
    CHOSEN_MAPS,
    parse_task,
    load_args,
    update_tasks,
...
from PLAP.llm_agents import LLMAgent
from PLAP.utils.metric import Metric
import PLAP.utils as utils
from gym_microrts import microrts_ai
from gym_microrts.envs.plap_vec_env import MicroRTSGridModePLAPVecEnv
from gym_microrts.envs.plap_vec_video_recorder import PLAPVecVideoRecorder
import warnings
...
def get_run_log_dir(args, engine, map_path):
    ...
    run_dir = (
        f"runs_{engine}_vs_rule/"
        if args.red in AI_MAPPING
        else f"runs_rule_vs_{engine}/"
    )
...
def init_environment(args, map_path, run_dir):
    ...
    env = MicroRTSGridModePLAPVecEnv(
        num_selfplay_envs=0,
        num_bot_envs=1,
