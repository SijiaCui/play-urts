...
from PLAP.grounding.task2actions import (
    TASK_DEPLOY_UNIT,
...
def task_assign(
    tasks: List[Tuple], obs_dict: Dict, path_planner: path_planning
) -> List[Dict]:
    ...
    for task in tasks:
        exist_units = ASSIGN_TASK_MAPPING[task[0]](
            task[1], exist_units, obs_dict, path_planner
        )
        ...
        for _unit in obs_dict[utils.FIGHT_FOR].values():
            if isinstance(_unit, dict) and _unit["type"] == "base":
                ...
                if len(return_locs) > 0:
                    return_loc = path_planner.get_manhattan_nearest(
                        unit["location"], return_locs
                    )
